...
function DataTable({ resource, rows, onRefund, onDisable }) {
  ...
  return <div className="overflow-x-auto rounded-xl border border-gray-100"><table className="min-w-full text-left text-xs"><thead className="bg-gray-50 text-[10px] uppercase tracking-wider text-gray-500"><tr><th className="px-4 py-3">Reference</th><th className="px-4 py-3">Application / product</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Amount / detail</th><th className="px-4 py-3">Updated</th>{['orders', 'licenses'].includes(resource) && <th className="px-4 py-3 text-right">Action</th>}</tr></thead><tbody className="divide-y divide-gray-100 bg-white">{rows.map((row, index) => {
    ...
    const detail = resource === 'orders' ? money(Number(first(row.total, row.amount, row.totalAmount, 0)) / 100, first(row.currency, row.currencyCode, 'USD')) : resource === 'subscriptions' ? `${row.cancelled ? 'Ends' : 'Renews'} ${dateTime(first(row.endsAt, row.renewsAt))}` : first(row.eventName, row.activationLimit != null ? `${row.activationUsage || 0}/${row.activationLimit} activations` : row.providerId, row.providerReference, '—');
...
export default function AdminPayments() {
  ...
  return <div className="space-y-6 animate-fade-in pb-10">
    ...
    <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-5"><Metric label="Net revenue" value={revenue ? money(Number(revenue.net || 0) / 100, revenue.currency) : '—'} icon={CircleDollarSign} tone="green" hint={testMode ? 'Sandbox totals only' : 'Paid orders less refunds'} /><Metric label="Paid orders" value={number(stats.orders?.primary)} icon={BadgeDollarSign} tone="orange" hint="Provider-confirmed only" /><Metric label="Active subscriptions" value={number(stats.subscriptions?.primary)} icon={Repeat2} tone="blue" hint="Cancelled grace periods stay active" /><Metric label="Active licenses" value={number(stats.licenses?.primary)} icon={KeyRound} tone="violet" hint="No full keys are shown" /><Metric label="Webhook health" value={stats.webhooks?.total ? `${Math.round((stats.webhooks.primary / stats.webhooks.total) * 100)}%` : '—'} icon={Webhook} tone="blue" hint="Processed webhook events" /></div>
